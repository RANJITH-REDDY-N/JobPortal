...
const UserDetailsModal = ({ user, onClose }) => {
  ...
  const downloadResume = () => {
    if (user.applicant_resume) {
      const link = document.createElement('a');
      link.href = user.applicant_resume;
      link.download = `resume_${user.user_first_name}_${user.user_last_name}.pdf`;
              ...
              {user.user_role === 'APPLICANT' && user.applicant_resume && (
                <div className={styles.resumeCard}>
                  ...
                  <div className={styles.resumeActions}>
                    ...
                    <button onClick={downloadResume} className={styles.resumeButton}>
                      <FiDownload size={16} />
                      Download
                    </button>
                  </div>
                  ...
                  {user.education.map((edu) => (
                    <div key={edu.education_id} className={styles.educationCard}>
                      <div className={styles.educationHeader}>
                        ...
                        <span className={styles.gpaBadge}>GPA: {edu.education_gpa}</span>
                      </div>
